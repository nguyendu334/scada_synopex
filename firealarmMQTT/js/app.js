...
// function read() {
//   getJSON("http://10.100.203.78:3004/data/1", function (err, data) {
//     if (err != null) {
//       console.error(err);
...
function myClientConnected() {
    MQTT_CLIENT.subscribe('SYNOPEXVINA/HIEUNV/MQTT/FIREALARMnew/');
...
// };
    ...
    try {
        ...
        for (var i = 0; i < 24; i++) {
            if (datashow[i] == 1) {
                document.getElementById('zone' + (i + 1) + '-fire').style.display = 'block';
                document.getElementById('zone' + (i + 1)).style.display = 'none';
            } else {
                document.getElementById('zone' + (i + 1) + '-fire').style.display = 'none';
                ...
                for (var i = 0; i < _iframe.length; i++) {
                    if (window.addEventListener) {
                        ...
                        document
                            ...
                            .contentWindow.document.addEventListener('touchstart', function () {
                                if (!once) {
                                    once = true;
                                    x.play();
                                }
